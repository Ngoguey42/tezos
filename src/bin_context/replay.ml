    ...
    let replay_config : Irmin_traces.Trace_replay.config =
      {
        ...
        keep_stats_trace = config.keep_stats_trace;
        no_summary = config.no_summary;
        empty_blobs = config.empty_blobs;
        stats_trace_message = config.stats_trace_message;
      }
    in
...
struct
  ...
  module Store = Maker.Make (Tezos_context_encoding.Context)
...
let main () ncommits ncommits_trace suite_filter inode_config store_type
    ...
    commit_data_file artefacts_dir keep_store keep_stats_trace no_summary
    empty_blobs startup_store_copy stats_trace_message =
...
let ncommits =
  let doc =
    Arg.info
      ~doc:"Number of commits for the large and chain modes."
      ["n"; "ncommits"]
  in
...
let keep_store =
  let doc =
    Arg.info
      ~doc:"Whether or not the irmin store on disk should be kept."
      ["keep-store"]
  in
  Arg.(value @@ flag doc)
...
let nchain_trees =
  let doc =
    Arg.info
      ~doc:"Number of chain trees per commit in chains-mode."
      ["c"; "nchain"]
  in
...
let nlarge_trees =
  let doc =
    Arg.info
      ~doc:"Number of large trees per commit in large-mode."
      ["l"; "nlarge"]
  in
  Arg.(value @@ opt int 1 doc)
...
let stats_trace_message =
  let doc =
    Arg.info
      ~docv:"MESSAGE"
      ~doc:"Untyped message to be stored in the header of the stats trace."
      ["stats-trace-message"]
  in
  Arg.(value @@ opt (some string) None doc)
...
let main_term =
  Term.(
    ...
    $ keep_store $ keep_stats_trace $ no_summary $ empty_blobs
    $ startup_store_copy $ stats_trace_message)
