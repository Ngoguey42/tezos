  ...
  let slice ~first ~last seq =
    let is_first_commit acc lvl_opt =
      ...
      | (`Idx i, _) when i < acc.commits_seen ->
          failwith "First block idx should be greater than 0"
      ...
      | (`Level _, Some _) ->
          failwith "Could not find requested first block level in input seq"
    in
    let is_last_commit acc lvl_opt =
      ...
      | (`Count _, _) ->
          failwith "The number of requested blocks should be greater than 0"
      | (`Level _, None) -> false
      | (`Level i, Some j) when i > j -> false
      | (`Level i, Some j) when i = j -> true
      | (`Level _, Some _) ->
          failwith "Could not find requested last block level in input seq"
    in
    let rec aux acc =
      match acc.seq () with
      | Seq.Nil -> (
          if acc.looking_for_first then
            failwith "Reached end of input seq before first commit"
          else
            match last with
            | `End -> Seq.Nil
            | `Count _ | `Level _ ->
      ...
      | Seq.Cons (ev, rest) -> (
          ...
          in
          match ev with
          | Def0.Commit _ | Commit_genesis _ ->
...
let event_infos =
  ...
  | Patch_context_exit (_, Error _) ->
      ...
      crash
  | Close | Sync _ | Set_master _ | Set_head _ ->
...
module Pass0 = struct
  ...
  module Segmentation = struct
    ...
    let rec look_for_block_start acc (idx, row) =
      ...
      match row with
      | Init (_, _) ->
          {acc with ingest_row = look_after_init idx; events_since_commit}
