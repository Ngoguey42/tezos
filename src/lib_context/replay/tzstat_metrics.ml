...
(** Handling of [tzstats.com] queries during conversion of a raw actions trace
    ...
    It might need to run using the [CONDUIT_TLS=openssl] env variable. *)
...
let fetch t i : unit Lwt.t =
  ...
  Array.iter
    (fun (height, op_count, op_count_tx, op_count_contract, hash) ->
      Hashtbl.add t.tbl height {op_count; op_count_tx; op_count_contract; hash})
    page
